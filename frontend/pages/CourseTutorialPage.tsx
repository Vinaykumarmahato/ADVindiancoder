...
const QuizCard = ({ 
    ...
    return (
        <div className={`group relative bg-white dark:bg-[#0d1117] border rounded-3xl overflow-hidden transition-all ${isSubmitted ? (selectedOption === correctIndex ? 'border-green-500/30' : selectedOption !== null ? 'border-red-500/30' : 'border-gray-200 dark:border-white/5') : 'border-gray-200 dark:border-white/5 hover:border-red-500/30'}`}>
            <div className="p-6 md:p-8">
                <div className="flex items-start gap-4 mb-8">
                    ...
                    <p className="text-gray-900 dark:text-white text-base md:text-lg leading-relaxed font-bold pt-1">
                        {question}
                    </p>
                </div>
                        ...
                        return (
                            <button
                                ...
                                className={`relative p-4 rounded-2xl text-left transition-all border flex items-center justify-between gap-4 ${
                                    isSelected 
                                        ? 'bg-red-600/10 border-red-500 text-red-600 dark:text-white' 
                                        : 'bg-gray-50 dark:bg-white/5 border-gray-200 dark:border-white/5 text-gray-700 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/10 hover:border-gray-300 dark:hover:border-white/10'
                                 } ${isSubmitted && isCorrect ? 'bg-green-600/10 border-green-500 text-green-650 dark:text-green-500' : ''} ${isSubmitted && isSelected && !isCorrect ? 'bg-red-600/10 border-red-500 text-red-650 dark:text-red-500' : ''}`}
                            >
                                <div className="flex items-center gap-3">
                                    <span className="w-6 h-6 rounded-lg bg-gray-200 dark:bg-white/5 flex items-center justify-center text-[10px] font-black uppercase">
                ...
                <div className="flex flex-col gap-4">
                    <button 
                        ...
                        className={`w-full py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 ${showAnswer ? 'bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-500 border border-gray-200 dark:border-white/10' : 'bg-gray-950 dark:bg-white text-white dark:text-black hover:bg-gray-900 dark:hover:bg-gray-200'}`}
                    >
                    ...
                    {showAnswer && (
                        <div className="p-6 bg-red-500/5 border border-red-500/10 rounded-2xl animate-in fade-in zoom-in duration-300">
                            <div className="flex items-center gap-2 mb-3 text-red-500 dark:text-red-400 font-black text-[10px] uppercase tracking-widest">
                                <span className="w-1.5 h-1.5 rounded-full bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]"></span>
                                Expert Solution
                            </div>
                            <div className="text-gray-600 dark:text-gray-300 text-sm md:text-base leading-loose mb-6">
                                {answer}
                            </div>
                            
                            <div className="pt-4 border-t border-gray-200 dark:border-white/5 flex items-center justify-between gap-4">
...
const CourseTutorialPage = () => {
    ...
    // Auto-enroll student into Java course on first visit if logged in
    useEffect(() => {
        if (!user) return;
        const progressList = user.courseProgressList || [];
        const isAlreadyEnrolled = progressList.some(p => p.courseId === 'java-full-course-2026');
            ...
            const enrollUser = async () => {
                ...
                try {
                    await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8080'}/api/enrollments/enroll`, {
                        ...
                        body: JSON.stringify({
                            courseId: 'java-full-course-2026',
                            courseName: 'Java Full Course 2026: Zero to Hero'
                        })
                    });
                    await refreshUser();
                } catch (e) {
    ...
    useEffect(() => {
        if (!user) return;
        const progress = (user.courseProgressList || []).find(p => p.courseId === 'java-full-course-2026');
        if (progress) {
            if (progress.quizStates) {
                try {
                    setQuizStates(JSON.parse(progress.quizStates));
                } catch (e) {
                    console.error("Failed to parse quiz states", e);
                }
            }
            if (progress.interviewRequested !== undefined) {
                setHasSentInterview(progress.interviewRequested);
            }
            if (progress.completedVideos) {
                const ids = progress.completedVideos.split(',')
    ...
    const syncProgressToBackend = async (updatedQuizStates: Record<number, any>, updatedInterviewRequested?: boolean) => {
        const token = localStorage.getItem('adv_coder_token');
        if (!token) return;

        let totalScoreSum = 0;
        let completedCountWithScore = 0;

        EPISODES.forEach(episode => {
            const parsed = updatedQuizStates[episode.id];
            if (parsed && parsed.score) {
                const pct = (parsed.score.correct / (episode.notes.quiz?.length || 1)) * 100;
                totalScoreSum += pct;
                completedCountWithScore++;
            }
        });

        const progressPercent = Math.round((completedVideoIds.length / EPISODES.length) * 100);
        const avgScore = completedCountWithScore > 0 ? Math.round(totalScoreSum / completedCountWithScore) : 0;
        ...
        try {
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8080'}/api/enrollments/update-progress`, {
                ...
                body: JSON.stringify({
                    ...
                    completedVideos: completedVideosStr,
                    assessmentScore: avgScore,
                    quizStates: JSON.stringify(updatedQuizStates),
                    interviewRequested: updatedInterviewRequested !== undefined ? updatedInterviewRequested : hasSentInterview
                })
            });
            await refreshUser();
        } catch (err) {
    ...
    const toggleVideoCompletion = async (episodeId: number) => {
        let updated: number[];
        if (completedVideoIds.includes(episodeId)) {
            updated = completedVideoIds.filter(id => id !== episodeId);
        } else {
            updated = [...completedVideoIds, episodeId];
        }
        ...
        const progressPercent = Math.round((updated.length / EPISODES.length) * 100);
        ...
        try {
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8080'}/api/enrollments/update-progress`, {
                ...
                body: JSON.stringify({
                    ...
                    interviewRequested: hasSentInterview
                })
            });
            await refreshUser();
        } catch (err) {
    ...
    const handleOptionSelect = (qIdx: number, optIdx: number) => {
        ...
        saveQuizProgress({ answers: newAnswers });
    };

    // Reporting State
    const [isReporting, setIsReporting] = useState(false);
    const [formData, setFormData] = useState({ name: '', email: '', mobile: '', portfolio: '' });
    ...
    const [hasSentInterview, setHasSentInterview] = useState(false);
    ...
    const handleInterviewRequest = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                syncProgressToBackend(quizStates, true);
            }
        } catch (error) {
    ...
    const certificateId = useMemo(() => {
        ...
        const nameHash = formData.name.split('').reduce((a: number, b: string) => { a = ((a << 5) - a) + b.charCodeAt(0); return a & a; }, 0);
    ...
    const averageScore = useMemo(() => {
        ...
        let count = 0;
        EPISODES.forEach(ep => {
            const saved = quizStates[ep.id];
            if (saved && saved.score) {
                const pct = (saved.score.correct / (ep.notes.quiz?.length || 1)) * 100;
                total += pct;
                count++;
            }
        });
    ...
    useEffect(() => {
        const activeQuizState = quizStates[ep.id];
        if (activeQuizState) {
            setUserAnswers(activeQuizState.answers || {});
            setIsSubmitted(activeQuizState.submitted || false);
            setScore(activeQuizState.score || { correct: 0, wrong: 0 });
            setHasSent(activeQuizState.hasSent || false);
        } else {
            setUserAnswers({});
    ...
    const saveQuizProgress = (data: any) => {
        ...
        syncProgressToBackend(updatedQuizStates);
    };
    ...
    const resetQuiz = () => {
        ...
        delete updatedQuizStates[ep.id];
        ...
        <PageWrapper>
            ...
            <div className="min-h-screen bg-white dark:bg-[#050914] text-gray-900 dark:text-white overflow-x-hidden transition-colors duration-300">
                ...
                <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-32 relative z-10">
                    ...
                    <div className="mb-8">
                        <h1 className="text-3xl md:text-5xl font-black mb-3 text-gray-905 dark:text-white">
                            Java Full Course <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-400">2026</span>
                        </h1>
                        <p className="text-gray-600 dark:text-gray-400 text-sm md:text-base flex items-center flex-wrap gap-x-4 gap-y-2">
                             Zero to Hero • <span className="text-gray-950 dark:text-white font-bold">{EPISODES.length} Episodes</span> • 100% Free
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                        ...
                        <div className="xl:col-span-2 flex flex-col gap-5">
                            ...
                            {/* Video Title + Tags */}
                            <div className="bg-white dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-gray-200 dark:border-white/10 p-6 md:p-8 shadow-sm dark:shadow-md">
                                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
                                    <h2 className="text-2xl md:text-3xl font-black text-gray-900 dark:text-white leading-tight flex-1">{ep.title}</h2>
                                    <div className="flex items-center gap-3 flex-shrink-0">
                                        ...
                                        <button
                                            onClick={() => toggleVideoCompletion(ep.id)}
                                            className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest px-5 py-3 rounded-xl transition-all hover:scale-105 active:scale-95 whitespace-nowrap shadow-lg cursor-pointer ${
                                                completedVideoIds.includes(ep.id)
                                                    ? 'bg-green-600 hover:bg-green-700 text-white shadow-green-600/20'
                                                    : 'bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/20 text-gray-800 dark:text-gray-200 border border-gray-250 dark:border-white/10'
                                            }`}
                                        >
                                            <CheckCircle className={`w-4 h-4 ${completedVideoIds.includes(ep.id) ? 'fill-white text-green-600' : ''}`} />
                                            {completedVideoIds.includes(ep.id) ? 'Watched ✓' : 'Mark Watched'}
                                        </button>
                                        <Link
                                            to="/adv-lab"
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    {ep.tags.map((tag, i) => (
                                        <span key={i} className="text-[10px] font-black tracking-widest text-red-650 dark:text-red-400 bg-red-500/10 border border-red-500/20 px-3 py-1.5 rounded-lg uppercase">
                                            {tag}
                                        </span>
                                    ))}
                                    <span className="text-[10px] font-black tracking-widest text-gray-600 dark:text-gray-500 bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 px-3 py-1.5 rounded-lg uppercase">
                            ...
                            {/* Notes Tabs */}
                            <div className="bg-white dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden shadow-sm dark:shadow-md">
                                {/* Tab Bar */}
                                <div className="flex bg-gray-100 dark:bg-black/40 p-1 rounded-2xl border border-gray-200 dark:border-white/5 backdrop-blur-md mb-8 overflow-x-auto scrollbar-hide">
                                    <button onClick={() => setActiveTab('notes')} className={`flex-1 flex items-center justify-center gap-2 py-3 px-6 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'notes' ? 'bg-red-600 text-white shadow-lg' : 'text-gray-600 dark:text-gray-500 hover:text-gray-950 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-white/5'}`}>
                                        <BookOpen className="w-4 h-4" /> Notes
                                    </button>
                                    <button onClick={() => setActiveTab('code')} className={`flex-1 flex items-center justify-center gap-2 py-3 px-6 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'code' ? 'bg-red-600 text-white shadow-lg' : 'text-gray-600 dark:text-gray-500 hover:text-gray-950 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-white/5'}`}>
                                        <Code2 className="w-4 h-4" /> Practice
                                    </button>
                                    <button onClick={() => setActiveTab('quiz')} className={`flex-1 flex items-center justify-center gap-2 py-3 px-6 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'quiz' ? 'bg-red-600 text-white shadow-lg' : 'text-gray-655 dark:text-gray-500 hover:text-gray-950 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-white/5'}`}>
                                ...
                                <div className="p-6">
                                    {activeTab === 'notes' && (
                                        <div className="space-y-5">
                                            <p className="text-gray-655 dark:text-gray-300 leading-relaxed text-sm md:text-base border-l-4 border-red-500 pl-4 py-1 bg-red-500/5 rounded-r-xl">
                                                {ep.notes.intro}
                                            </p>
                                            <h3 className="text-gray-900 dark:text-white font-bold text-base">📚 Topics Covered</h3>
                                    ...
                                    {activeTab === 'code' && (
                                        <div>
                                            <div className="flex items-center gap-2 mb-3">
                                                ...
                                                <span className="text-xs text-gray-600 dark:text-gray-500 font-mono">EP{String(ep.id).padStart(2,'0')}.java</span>
                                            </div>
                                            <pre className="bg-gray-50 dark:bg-[#0d1117] text-green-700 dark:text-green-300 text-sm p-4 rounded-xl overflow-x-auto leading-relaxed font-mono border border-gray-200 dark:border-white/10 whitespace-pre-wrap">
                                                {ep.notes.code}
                                            </pre>
                                        </div>
                                    )}
                                    ...
                                    {activeTab === 'quiz' && (
                                        <div className="space-y-6">
                                            <div className="flex items-center justify-between mb-2">
                                                <div>
                                                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-1">Interactive MCQ Quiz</h3>
                                                    <p className="text-xs text-gray-600 dark:text-gray-500">Test your mastery of this lesson's topics</p>
                                                </div>
                                                <div className="text-right">
                                                    <span className="text-2xl font-black text-red-500">
                                                        {ep.notes.quiz?.length || 0}
                                                    </span>
                                                    <span className="text-xs text-gray-605 dark:text-gray-500 ml-1">Questions</span>
                                                </div>
                                            </div>

                                            {isSubmitted && (
                                                <div className="p-8 rounded-[2rem] bg-white dark:bg-black/20 border border-gray-200 dark:border-white/5 backdrop-blur-2xl animate-in fade-in slide-in-from-top-4 duration-500 mb-8 shadow-sm dark:shadow-md text-gray-905 dark:text-white">
                                                    <div className="flex flex-col md:flex-row items-center gap-8">
                                                        <div className="relative w-32 h-32 flex items-center justify-center">
                                                            <svg className="w-full h-full -rotate-90">
                                                                <circle cx="64" cy="64" r="58" fill="transparent" stroke="rgba(0,0,0,0.05)" className="dark:stroke-white/5" strokeWidth="8" />
                                                                <circle 
                                                            ...
                                                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                                <span className="text-3xl font-black text-gray-950 dark:text-white">{Math.round((score.correct / (ep.notes.quiz?.length || 1)) * 100)}%</span>
                                                                <span className="text-[10px] text-gray-600 dark:text-gray-500 uppercase font-black">Score</span>
                                                            </div>
                                                        </div>
                                                        <div className="flex-1 text-center md:text-left">
                                                            <h4 className="text-2xl font-black text-gray-950 dark:text-white mb-2">Quiz Performance</h4>
                                                            <div className="grid grid-cols-3 gap-4 mt-4">
                                                                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/5">
                                                                    <div className="text-[10px] text-gray-600 dark:text-gray-500 uppercase font-black mb-1">Correct</div>
                                                                    <div className="text-xl font-black text-green-550 dark:text-green-500">{score.correct}</div>
                                                                </div>
                                                                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/5">
                                                                    <div className="text-[10px] text-gray-600 dark:text-gray-500 uppercase font-black mb-1">Wrong</div>
                                                                    <div className="text-xl font-black text-red-550 dark:text-red-500">{score.wrong}</div>
                                                                </div>
                                                                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/5">
                                                                    <div className="text-[10px] text-gray-600 dark:text-gray-500 uppercase font-black mb-1">Total</div>
                                                                    <div className="text-xl font-black text-blue-550 dark:text-blue-500">{ep.notes.quiz?.length || 0}</div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div className="flex flex-col gap-3">
                                                            {!hasSent ? (
                                                                ...
                                                                <div className="px-8 py-4 bg-green-500/10 border border-green-500/20 text-green-600 dark:text-green-500 font-black text-xs uppercase tracking-widest rounded-full flex items-center gap-2">
                                                                    <CheckCircle2 className="w-4 h-4" /> Result Submitted
                                                                </div>
                                                            )}
                                                            <button 
                                                                onClick={resetQuiz}
                                                                className="px-8 py-4 bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-400 font-black text-xs uppercase tracking-widest rounded-full hover:bg-gray-200 dark:hover:bg-white/10 transition-all"
                                                            >
                                                                Restart Quiz
                                                            </button>
                                                        </div>
                                                    ...
                                                    {isReporting && (
                                                        <div className="mt-8 p-8 border-t border-gray-200 dark:border-white/10 animate-in fade-in slide-in-from-top-4 duration-500">
                                                            <h4 className="text-xl font-black text-gray-950 dark:text-white mb-6 flex items-center gap-2">
                                                                <Library className="w-5 h-5 text-red-500" />
                                                                Submit Performance Report
                                                            </h4>
                                                            <form onSubmit={handleReportSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 text-gray-905 dark:text-white">
                                                                <div className="space-y-2">
                                                                    <label className="text-[10px] font-black text-gray-600 dark:text-gray-500 uppercase tracking-widest ml-1">Full Name</label>
                                                                    <input 
                                                                        type="text" required value={formData.name}
                                                                        onChange={e => setFormData({ ...formData, name: e.target.value })}
                                                                        className="w-full bg-gray-50 dark:bg-[#0d1117] border border-gray-200 dark:border-white/5 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white focus:border-red-500 outline-none transition-colors"
                                                                        placeholder="Your Name"
                                                                    />
                                                                </div>
                                                                <div className="space-y-2">
                                                                    <label className="text-[10px] font-black text-gray-600 dark:text-gray-500 uppercase tracking-widest ml-1">Email Address</label>
                                                                    <input 
                                                                ...
                                                                <div className="space-y-2">
                                                                    <label className="text-[10px] font-black text-gray-600 dark:text-gray-500 uppercase tracking-widest ml-1">Mobile Number</label>
                                                                    <input 
                                                                ...
                                                                <div className="md:col-span-3 mt-4 flex justify-end gap-3">
                                                                    <button 
                                                                        ...
                                                                        className="px-6 py-3 text-xs font-black text-gray-600 dark:text-gray-500 uppercase tracking-widest hover:text-gray-950 dark:hover:text-white transition-colors"
                                                                    >
                                                                        Cancel
                                                                    </button>
                                                                    <button 
                                            ...
                                            <div className="mt-16 p-8 rounded-[2rem] bg-gradient-to-br from-red-500/5 via-transparent to-blue-550/5 border border-gray-250 dark:border-white/5 text-center backdrop-blur-2xl text-gray-905 dark:text-white">
                                                <div className="w-16 h-16 bg-red-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
                                                    <Library className="w-8 h-8 text-red-500" />
                                                </div>
                                                <h4 className="text-xl md:text-2xl font-black text-gray-950 dark:text-white mb-2">Mastered the Theory?</h4>
                                                <p className="text-gray-655 dark:text-gray-400 mb-8 max-w-md mx-auto">Theory is only half the battle. Jump into the Lab and build something real!</p>
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className="flex-1 py-3 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 text-gray-800 dark:text-gray-300 disabled:opacity-30 disabled:cursor-not-allowed transition-all font-semibold text-sm shadow-sm"
                                >
                                    ← Previous Episode
                                </button>
                                <button
                                    ...
                                    className="flex-1 py-3 rounded-xl border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 disabled:opacity-30 disabled:cursor-not-allowed transition-all font-semibold text-sm text-red-600 dark:text-red-300 shadow-sm"
                                >
                                    Next Episode →
                                </button>
                            </div>
                        ...
                        <div className="xl:col-span-1">
                            <div className="bg-white dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-gray-200 dark:border-white/10 flex flex-col xl:sticky xl:top-32 xl:max-h-[80vh] overflow-hidden shadow-sm dark:shadow-md">
                                <div className="p-6 border-b border-gray-250 dark:border-white/10 flex items-center justify-between bg-gradient-to-r from-red-600/10 to-transparent relative text-gray-900 dark:text-white">
                                    <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-red-600 via-red-600/50 to-transparent"></div>
                                    <h3 className="font-black flex items-center gap-2 text-sm uppercase tracking-widest text-gray-950 dark:text-white">
                                        <Library className="w-4 h-4 text-red-500" /> Curriculum
                                    </h3>
                                    <span className="text-[10px] font-black text-gray-600 dark:text-gray-500 bg-gray-100 dark:bg-white/5 px-2 py-1 rounded-md border border-gray-200 dark:border-white/5">
                                                ...
                                                <div className="flex flex-col justify-center flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        ...
                                                        {completedVideoIds.includes(video.id) && (
                                                            <CheckCircle className="w-3 h-3 text-green-500 fill-green-500/10 shrink-0" />
                                                        )}
                    ...
                    <div className="mt-16 pt-16 border-t border-white/5 grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
                        ...
                        {/* Verify Section */}
                        <div className="p-8 rounded-3xl bg-white dark:bg-[#0d1117] border border-gray-200 dark:border-white/5 shadow-sm dark:shadow-md text-gray-905 dark:text-white backdrop-blur-xl flex flex-col justify-between">
                            <div>
                                <div className="flex items-center gap-2 mb-4">
                                    <ShieldCheck className="w-5 h-5 text-green-500" />
                                    <span className="text-[10px] font-black text-gray-955 dark:text-white uppercase tracking-widest">Verify a Certificate</span>
                                </div>
                                <div className="space-y-3">
                                    <input 
                                        type="text" 
                                        placeholder="VERIFY CREDENTIAL ID" 
                                        className="w-full bg-gray-50 dark:bg-black/20 border border-gray-250 dark:border-white/10 rounded-xl px-4 py-3 text-[10px] font-mono text-gray-900 dark:text-white focus:border-red-500 outline-none transition-all placeholder:text-gray-400 dark:placeholder:text-gray-700 uppercase"
                                        value={verifyId}
                                        onChange={e => {
                                    ...
                                    <button 
                                        ...
                                        className={`w-full py-3 rounded-xl font-black text-[9px] uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 ${
                                            ...
                                            'bg-gray-955 dark:bg-white text-white dark:text-black hover:bg-gray-900 dark:hover:bg-gray-200'
                                        }`}
                                    >
                            ...
                            <p className="text-[9px] text-gray-550 dark:text-gray-600 font-bold italic text-center leading-tight mt-4">
                                Academic credentials are cryptographically tied to the profile.
                            </p>
                        </div>

                        {/* Rulebook Section */}
                        <div className="p-8 rounded-3xl bg-white dark:bg-[#0d1117] border border-gray-200 dark:border-white/5 shadow-sm dark:shadow-md text-gray-905 dark:text-white backdrop-blur-xl">
                            <h4 className="text-[9px] font-black text-gray-600 dark:text-gray-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                            ...
                            <div className="space-y-3">
                                <div className="flex gap-3">
                                    ...
                                    <div className="space-y-0.5">
                                        <div className="text-[10px] font-black text-gray-955 dark:text-white uppercase tracking-wider">Submit Assessments</div>
                                        <p className="text-[9px] text-gray-600 dark:text-gray-500 font-medium leading-relaxed">Complete and submit the quizzes for all curriculum modules sequentially.</p>
                                    </div>
                                </div>
                                <div className="flex gap-3">
                                    <span className="text-[9px] font-black text-red-500 opacity-50 shrink-0">02</span>
                                    <div className="space-y-0.5">
                                        <div className="text-[10px] font-black text-gray-955 dark:text-white uppercase tracking-wider">Eligibility & Group Access</div>
                                        <p className="text-[9px] text-gray-600 dark:text-gray-500 font-medium leading-relaxed">Unlock the application form and secure the opportunity to join our official certificates hub.</p>
                                    </div>
                                </div>
                                <div className="flex gap-3">
                                    <span className="text-[9px] font-black text-red-500 opacity-50 shrink-0">03</span>
                                    <div className="space-y-0.5">
                                        <div className="text-[10px] font-black text-gray-955 dark:text-white uppercase tracking-wider">15-Min F2F Interview</div>
                                        <p className="text-[9px] text-gray-600 dark:text-gray-500 font-medium leading-relaxed">Schedule a quick 15-minute face-to-face capability evaluation with mentorship mentors.</p>
                                    </div>
                                </div>
                                <div className="flex gap-3">
                                    <span className="text-[9px] font-black text-red-500 opacity-50 shrink-0">04</span>
                                    <div className="space-y-0.5">
                                        <div className="text-[10px] font-black text-gray-955 dark:text-white uppercase tracking-wider">Receive Credential</div>
                                        <p className="text-[9px] text-gray-600 dark:text-gray-500 font-medium leading-relaxed">Clear the technical meet successfully to receive your industrial-grade credential.</p>
                                    </div>
                                </div>
                            </div>
                            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-white/5 text-right">
                    ...
                    {isCourseCompleted && (
                        <div className="mt-12 p-8 rounded-[2rem] bg-gradient-to-br from-green-500/5 to-red-500/5 border border-green-500/20 max-w-4xl mx-auto relative z-10 animate-in fade-in slide-in-from-top-4 duration-500 shadow-sm dark:shadow-md text-gray-905 dark:text-white">
                            <div className="text-center mb-8">
                                <Trophy className="w-16 h-16 text-green-500 mx-auto mb-4 animate-bounce" />
                                <h4 className="text-3xl font-black text-gray-950 dark:text-white mb-2">Curriculum 100% Cleared</h4>
                                <p className="text-green-550 dark:text-green-400 font-bold text-sm tracking-widest uppercase">Eligible for Graduation Interview</p>
                            </div>

                            <form onSubmit={handleInterviewRequest} className="space-y-6 max-w-2xl mx-auto p-6 bg-white dark:bg-black/40 rounded-3xl border border-gray-200 dark:border-white/5 shadow-sm dark:shadow-md text-gray-905 dark:text-white">
                                <h5 className="text-lg font-black text-gray-950 dark:text-white flex items-center gap-2">
                                    <Info className="w-5 h-5 text-red-500" /> Complete your evaluation profile
                                </h5>
                                
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-600 dark:text-gray-500 uppercase tracking-widest ml-1">Full Legal Name</label>
                                        <input 
                                            type="text" required
                                            value={formData.name}
                                            onChange={e => setFormData({ ...formData, name: e.target.value })}
                                            className="w-full bg-gray-50 dark:bg-[#0d1117] border border-gray-200 dark:border-white/5 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white focus:border-red-500 outline-none transition-all"
                                            placeholder="Name on certificate"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-600 dark:text-gray-500 uppercase tracking-widest ml-1">Mobile / WhatsApp Number</label>
                                        <input 
                                            type="tel" required
                                    ...
                                    <div className="space-y-2 md:col-span-2">
                                        <label className="text-[10px] font-black text-gray-600 dark:text-gray-500 uppercase tracking-widest ml-1">Project/GitHub Links (Proof of Work)</label>
                                        <input 
                                            type="text"
